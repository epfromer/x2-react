...
export default class MUIGravatar2 extends Avatar {
  static displayName = 'MUIGravatar2'
  static propTypes = {
  ...
  render() {
    ...
    if (this.props.md5) {
      ...
      console.warn(
        'MUIGravatar2 image can not be fetched. Either the "email" or "md5" prop must be specified.'
      )
      return <script />
    }
    ...
    if (!modernBrowser && isRetina()) {
      return (
        <img
          alt={
            this.props.email
              ? `MUIGravatar2 for ${formattedEmail}`
              : `MUIGravatar2`
          }
    ...
    return (
      <img
        alt={`MUIGravatar2 for ${formattedEmail}`}
        style={this.props.style}
        src={src}
        srcSet={`${retinaSrc} 2x`}
        height={this.props.size}
